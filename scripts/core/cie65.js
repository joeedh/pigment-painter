...
for (let di = 0; di < data.length; di += DTOT) {
  range[0] = Math.min(range[0], data[di + DWAVELEN]);
  range[1] = Math.max(range[1], data[di + DWAVELEN]);
}

//nanometers
export function waveLengthToFreq(w) {
  w *= 1e-9; //to meters

  return 299792458.0/w;
}

export function freqToWaveLength(f) {
  f = 299792458.0/f;
...
let frange = [waveLengthToFreq(range[0]), waveLengthToFreq(range[1])];

let steps = 1024;
let table = new Array(steps);
let used = new Array(steps);
for (let i = 0; i < steps; i++) {
  used[i] = 0.0;
}

for (let di = 0; di < data.length; di += DTOT) {
  let wlen = data[di + DWAVELEN];
  let freq = waveLengthToFreq(wlen);

  //console.log(freq);
  let t = (freq - frange[0])/(frange[1] - frange[0]);
  let ti = ~~(t*steps*0.9999);

  if (used[ti] === 0.0) {
    table[ti] = [data[di + DS1], data[di + DS2], data[di + DS3]];
  } else {
...
for (let i = 0; i < steps - 1; i++) {
  if (table[i + 1] !== undefined || table[i] === undefined) {
    continue;
  }

  let j = i + 1;
  while (j < steps && table[j] === undefined) {
    j++;
  }
...
export const colorTemperature = 6504;

export function getCie65(freq) {
  let i = (freq - frange[0])/(frange[1] - frange[0]);
  i = Math.min(Math.max(i, 0.0), 1.0)*0.99999;
  ...
  ret.s1 = table[i1][0] + (table[i2][0] - table[i1][0])*t;
  ret.s2 = table[i1][1] + (table[i2][1] - table[i1][1])*t;
  ret.s3 = table[i1][2] + (table[i2][2] - table[i1][2])*t;
